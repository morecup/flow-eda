...
XML 生成节点主要用于基于指定的 XML 模板生成新的 XML 内容，并支持通过传入参数覆盖模板中的节点值或属性值。

**节点参数：**

- 模板文件<font color=red>*</font>：XML 模板文件的定位方式。支持 `a.xml`（默认从 `classpath:xml-template/` 下读取），或使用 `classpath:`/`file:` 前缀指定完整路径。
- 覆盖参数：用于覆盖模板中的值，要求为 json 格式的 key-value 映射；key 为“节点路径”，value 为要覆盖的新值。
- 输出文件路径：若填写则会把生成后的 XML 写入该路径（UTF-8，覆盖写）；支持占位符 `${filePath}` 等，且会自动创建上级目录。

**外部参数传递：**

- 运行流程时可通过接口 `POST /api/v1/node/data/run` 的 `payload` 字段传入参数；这些参数会作为下游节点的 `input`，可被占位符解析。
- 在 `overrides` 的 value 中使用占位符 `${参数名}` 引用外部参数，例如：`{"Header.To":"${to}","Body.Amount":"${amount}"}`。
- 若需要写入文件，可将节点参数 `filePath` 配置为 `${filePath}`，并在运行请求的 `payload.filePath` 中传入真实路径。
- 若为嵌套对象，可使用 `${a.b.c}` 的方式取值。
...
- 节点路径支持 `.` 或 `/` 分隔，例如 `Header.To` 或 `/Root/Header/To`。
- 支持数组下标：`Item[0]`（从 0 开始）。
- 覆盖属性：使用 `@`，例如 `Body.@id` 或 `Body/@id`。

**输出：**

- 输出字段 `result` 为生成后的 XML 字符串。
