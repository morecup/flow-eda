package com.flow.eda.runner.flow.node.data;
...
public interface NodeDataMapper {
    ...
    @Insert(
            "<script>INSERT INTO flow_node_data (id, node_name, flow_id, type_id, `top`, `left`, remark, "
    ...
    @Select(
            "<script>SELECT * FROM flow_node_data WHERE flow_id=#{flowId} "
    ...
    @Select(
            "SELECT version FROM flow_node_data WHERE flow_id=#{flowId} "
                    + "AND version IS NOT NULL GROUP BY version ORDER BY version DESC")
    List<String> findVersionsByFlowId(String flowId);

    @Delete("DELETE FROM flow_node_data WHERE flow_id=#{flowId}")
    void deleteByFlowId(String flowId);

    @Delete("DELETE FROM flow_node_data WHERE flow_id=#{flowId} AND version IS NULL")
